...
def run_full_refresh(master_csv: Path, nlp_batch_size: int = 1000) -> dict[str, str]:
    ...
    from src.nlp.nlp_pipeline import run_stage_3_nlp_extraction
    ...
    stage3_output = PROCESSED_DATA_DIR / f"jobpulse_nlp_{run_id}.parquet"
    logger.info("Stage 3/4 — extracting skills and metadata: %s", stage2_output)
    run_stage_3_nlp_extraction(Path(stage2_output), stage3_output, batch_size=nlp_batch_size)

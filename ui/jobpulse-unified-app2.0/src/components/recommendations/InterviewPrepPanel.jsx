  ...
  if (prepItems.length === 0) {
    return (
      <div className="rounded-2xl border p-6 text-center" style={{ borderColor: COLORS.border, background: "#fff" }}>
        <MessageSquare size={24} style={{ color: COLORS.textMuted, margin: "0 auto 8px" }} />
        <p className="text-sm" style={{ color: COLORS.textSecondary }}>
          No interview prep available right now. Check back later as new resources are added regularly.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-0.5 text-xs" style={{ color: COLORS.textSecondary }}>
          Practice mock interviews for the skills you already have
        </p>
      </div>
        ...
        {prepItems.map((item, i) => (
          ...
          >
            <div>
              ...
              <p className="mt-2 text-xs leading-relaxed" style={{ color: COLORS.textSecondary }}>{item.reason}</p>
            </div>
